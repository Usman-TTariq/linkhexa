...
export default function LostTransactionsContent() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/awin/lost-transactions?limit=${limit}&offset=${offset}`, {
        credentials: "include",
      });
  ...
  const [assignFor, setAssignFor] = useState<LostRow | null>(null);

  const end = Math.min(offset + rows.length, offset + limit);
      ...
      <div className="px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold tracking-tight text-white">Lost transactions</h1>
            <p className="mt-1 max-w-2xl text-sm text-zinc-400">
              Rows in <code className="rounded bg-white/10 px-1 text-xs">awin_transactions</code> with no{" "}
              <code className="rounded bg-white/10 px-1 text-xs">publisher_id</code> after sync and fallbacks. Assign to credit the
              correct publisher; the row is marked manual so the next sync does not overwrite attribution.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link
              href="/admin/awin/all-transactions"
        ...
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/60 shadow-lg shadow-black/20">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
            <p className="text-sm text-zinc-400">
              <span className="font-semibold text-white">{total.toLocaleString()}</span> lost{" "}
              {total === 1 ? "row" : "rows"}
              {loading ? " · loading…" : rows.length ? ` · showing ${offset + 1}–${end}` : ""}
            </p>
            ...
            <table className="min-w-full divide-y divide-white/10 text-left text-sm">
              <thead className="bg-zinc-950/50 text-xs uppercase tracking-wider text-zinc-500">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium">Click ref</th>
                ...
                {!loading && rows.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-10 text-center text-zinc-500">
                      No lost transactions. Everything in the table has a publisher, or the table is empty.
                    </td>
                  </tr>
                ) : (
                  rows.map((r) => (
                    <tr key={r.awin_transaction_id} className="hover:bg-white/[0.03]">
                      ...
                      <td className="max-w-[200px] truncate px-4 py-3 text-zinc-300" title={r.advertiser_name ?? undefined}>
                      ...
                      <td className="max-w-[140px] truncate px-4 py-3 font-mono text-xs text-zinc-500" title={r.click_ref ?? ""}>
                        {r.click_ref?.trim() ? r.click_ref : "—"}
                      </td>
